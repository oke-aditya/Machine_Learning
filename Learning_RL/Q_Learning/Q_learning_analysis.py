...
LEARNING_RATE = 0.1
DISCOUNT = 0.95     # How much we want to reward for future
EPISODES = 25000
SHOW_EVERY = 3000      # Just to check
epsilon = 0.8          # Degree of random exploration
...
for episode in range(EPISODES):
	...
	if not (episode % SHOW_EVERY):
		...
		print("Episode: %d avg: %0.2f: min: %d: max: %d" %(episode, average_reward, min(ep_rewards[-SHOW_EVERY:]), max(ep_rewards[-SHOW_EVERY:])))

	if(episode % 10 == 0):
		np.save(f"qtables{episode}.npy", q_table)
