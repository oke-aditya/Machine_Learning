...
Original file is located at
    https://colab.research.google.com/drive/13e_khmOqSN9QJaqfVnI9NA5ip544cy2E
...
def load(image_file):
    ...
    # Image is having 2 images side by side 
    w = tf.shape(image)[1] # Width of the image
    w = w // 2
    real_image = image[:, :w, :]
    input_image = image[:, w:, :]
...
def load_image_test(image_file):
    input_image, test_image = load(image_file)
    input_image, test_image = resize(input_image, test_image, IMG_HEIGHT, IMG_WIDTH)
    input_image, test_image = normalize(input_image, test_image)
...
down_result = down_model(tf.expand_dims(inp, 0)) # For batch size
...
gen_output = generator(inp[tf.newaxis,...], training=False) # Adding extra dim for batch size
...
The training=True is intentional here since we want the batch statistics while running the model on the test dataset. If we use training=False, we will get the accumulated statistics learned from the training dataset (which we don't want)
"""
